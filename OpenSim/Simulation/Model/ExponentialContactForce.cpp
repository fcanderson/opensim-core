...
ExponentialContactForce(const ExponentialContactForce& source)
{
    ...
    // There's no need to explicitly connect the station socket as that
    // is handled by the copy assignments in the parent classes (I think).
    // So, I have commented out the following line. Tests are passing.
    // Uncommenting the line results in exceptions being thrown.
    ...
    // left as nullptr, which prevents double deletion the SimTK Subsystem.
}
...
operator=(const ExponentialContactForce& source) {
    if (this != &source) {
        // Set the contact plane transform only if the current instance
        // has not been added to the SimTK::System.
        if (this->_spr == nullptr) {
            set_contact_plane_transform(source.get_contact_plane_transform());
        }
        // The following call will push the parameters to the properties.
