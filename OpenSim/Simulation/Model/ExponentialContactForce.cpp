...
ExponentialContactForce(const ExponentialContactForce& source)
{
    ...
    // Establish member variable equality in the parent classes.
    // The copy assignment operators in the parent classes are automatically
    // generated by the compiler and will copy the properties from the source
    // instance parent classes.
    this->Super::operator=(source);

    // Copy the properties from this instance.
    set_contact_plane_transform(source.get_contact_plane_transform());
    setParameters(source.getParameters());

    // I think there's no need to explicitly connect the station socket as that
    // is handled by the copy assignments in the base classes, so I have
    // commented out the following line. Uncommenting the line results in
    // exceptions being thrown. So, perhaps the station socket is not
    // functional.
    //connectSocket_station(source.getConnectee<Station>("station"));

    // Note that the underlying SimTK::ExponentialSpringForce pointer is
    // left as nullptr, which will prevent double deletion of the pointer.
}
...
operator=(const ExponentialContactForce& source) {
    if (this != &source) {
        // Set the contact plane transform and station connection only if the
        // current instance is has not been added to the SimTK::System.
        if (this->_spr == nullptr) {
            set_contact_plane_transform(source.get_contact_plane_transform());
            connectSocket_station(source.getConnectee<Station>("station"));
        }
        // The following call will also push the parameters to the properties.
        setParameters(source.getParameters());
    }
    return *this;
}
...
ExponentialContactForce(ExponentialContactForce&& source) noexcept {
    ...
    // The following call will also push the parameters to the properties.
    setParameters(source.getParameters());
    // Properly move the SimTK::ExponentialSpringForce pointer (_spr)
    this->_spr = source._spr;
    // Set the source pointer to nullptr so the delete does not happent twice
    // (i.e., once in the source and again in the new instance).
    source._spr = nullptr;
}
*/
